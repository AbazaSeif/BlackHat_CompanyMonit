...
public class PCapFileHeader
{
    ...
    {
		uint32MagicNum = PCapFileReader.MAGIC_NUMBER_DONT_FLIP;
	...
	public void readHeader(InputStream in) throws IOException
	{
		...
		ushort16VersionMajor = ByteUtils.getByteNetOrderTo_uint16(tmp, 4);
		ushort16VersionMinor = ByteUtils.getByteNetOrderTo_uint16(tmp, 6);
		...
		if (uint32MagicNum == PCapFileReader.MAGIC_NUMBER_DONT_FLIP)
		{
			isflip = false;
        }
		else if (uint32MagicNum == PCapFileReader.MAGIC_NUMBER_FLIP)
		{
			isflip = true;
		}
		else
