...
public class FlowTCP extends Flow 
{
	...
	/**
 	 ...
 	 * 
 	 */
	class TcpFlowPacketIterator implements Iterator<TCPPacket>
	{
		private Iterator<Integer> myItr = null;
		
		private TcpFlowPacketIterator()
		{
		...
		{
			int n = myItr.next();
			System.out.println("cap n = : "+n);
			return new TCPPacket(myCaptureFileAnalyzer.getPktNum(n));
		}
	...
	public TCPPacket [] getReasmble(PacketSide side)
	{
		
		ArrayList<TCPPacket> pkts = new ArrayList<TCPPacket>();
		ArrayList<Integer> pktsi = new ArrayList<Integer>();
		HashMap<TCPPacket, Integer> map = new HashMap<TCPPacket, Integer>();
		for(int i=1 ; i<= getNumOfPkts() ; i++)
		{
			TCPPacket tcppkt = getTCPPkt(i);
			
			if(myState.getPktSide(tcppkt) == side && tcppkt.getPayloadDataLength() > 0)
			{
				//System.out.println(getPktGlobalIdx(i));
				pktsi.add(getPktGlobalIdx(i));
				pkts.add(tcppkt);
				map.put(tcppkt, getPktGlobalIdx(i));
			}
		}
		
		Collections.sort(pkts, new Comparator<TCPPacket>() {
		...
		int n = 0;
		//System.out.println("pkts before : "+pkts.size());
		for(int i=0 ; i<pkts.size()-1 ; n++)
		{
			if(pkts.get(i).getPayloadDataLength() == 0)
			{
				pkts.remove(i);
				//System.out.println("removing none payload: "+pktsi.get(n));
				continue;
			}
			if (pkts.get(i).getSequenceNumber() == pkts.get(i+1).getSequenceNumber())
			{
				//System.out.println("match --- "+pkts.get(i).getSequenceNumber());
				//System.out.println("removing dup: "+map.get(pkts.get(i))+","+map.get(pkts.get(i+1)));
				pkts.remove(i);
				continue;
			}
			i++;
		...
		//System.out.println("pkts after : "+pkts.size());
		
		return pkts.toArray(new TCPPacket[pkts.size()]);
	}
